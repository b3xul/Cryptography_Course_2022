...
if __name__ == '__main__':
    ...
    server.send(edt)
    responseBAD = server. recv(1024)
    print(responseBAD)
    server.close()

#---------------------------------------------
    print(len(ciphertext)//AES.block_size) # floor division
    N = len(ciphertext)//AES.block_size # ciphertext composed of 4 Blocks
    initial_part = ciphertext[:(N-2)*AES.block_size] # leave all blocks unmodified except last 2
    ...
    p_prime_15 = 189  # Partial solution until now
    print("---------------")
