...
# -------------------
    ...
    m2 = hashlib.sha1(dgst) # count of message bits processed so far
    m2.update(additional_part)
    print(m2.hexdigest())
    #m2.hexdigest() should be the digest of test_secret + message + to_add

    m3 = hashlib.sha1()
    pad = padding(len(key+msg)*8)
    print(pad)
    print(key+msg+ pad + additional_part)
    m3.update(key+msg+ pad + additional_part) # padding len(test_secret+msg+ pad + to_add)
    print(m3.hexdigest())
