...
def guess_byte(p, c, ciphertext, block_size):
    # x923 padding: 5 real block | 11 empty block -> "0*11" "11"
    # x923 padding: 9 real block | 7 empty block -> "0*7" "7"
    ...
    # 0004
    # 00005
    final_padding_value = len(p) + 1
    intermediate_padding = 0
    print("pad=" + str(final_padding_value))
    ...
    padding_value = 0
    if (current_byte_index % block_size) == block_size - 1:
        padding_value = 1
    ...
    for i in range(0, 256):
        ...
        for x in p:
            if j == len(p):
                ca += (x ^ final_padding_value).to_bytes(1, byteorder='big')
            else:
                ca += (x ^ intermediate_padding).to_bytes(1, byteorder='big')
            j += 1
        # print(ca)
...
if __name__ == '__main__':
    ...
    ciphertext = b"\x94\x99d{SE,\x00I|\x04\xfe\x14N5\xb4\x1c\xadk\x9f\xfc\xceg8'\xe6{\x13\xc9\x94\xce\xae\x0b\x97\xfbm\x8b0\n\\\x94-~\xa7CH\xbf\x150\x88:(\x8c\xeb\xbf5\xaf\xdd_5\xe9\xb4f\xf9"
