...
while 1:
    ...
    # Server context: SRPContext(auth_data['username'], prime=auth_data['prime'], generator=auth_data['generator']
    # is just a partial context while
    # Client context: srp_context = SRPContext(username,password)
    # is a full context since it knows the password
    ...
    print("server public= " + str(server_public_B))


    server_data = json.dumps({'server_verifier': server_public_B}) # 'server_salt' : server_salt})
    # print(server_data)
