...
int main(int argc, char **argv) {
    ...
    // passphrase to decrypt file containing private key
...
// no private key context needed because we are not computing the keyed digest, but just a digital signature
    ...
    while((n_read = fread(buffer,1,MAXBUFFER,f_in)) > 0){
      // feed 1024 bytes at a time to the digest update function
    ...
    // Pad data and compute digest
    if(!EVP_DigestSignFinal(sign_ctx, NULL, &digest_len))
        handle_errors();  


    // Compute signature on previously computed digest
